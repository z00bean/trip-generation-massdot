...
def draw_boxes(img, f_no, bbox, names,object_id, identities=None, offset=(0, 0)):
    ...
    if len(temp1) > 0:
        for p_key in temp1:
            ...
            if p_key not in identities and len(identities) > 0:
                ...
                if abs(max(dict_purp_Y[p_key]) - min(dict_purp_Y[p_key])) < 900 and len(dict_purp_Y[p_key]) >= 1:#int(fps_vid/10):
                    #if short trajectory appears in the middle of a video (5 seconds after begin), it is probably a tracking error.
                    # B4-434, 04120158.MOV
                    if f_no > fps_vid*5 and len(dict_purp_Y[p_key]) < int(fps_vid/2):
    ...
    for i, box in enumerate(bbox):
        ...
        if getMaskPixelColor(int(center[1]), int(center[0])) == "yellow" and id not in dict_yellow.keys() and id not in listVehCounted:
            ...
            if id in dict_purple.keys():
                ...
                if obj_name not in object_counter1:
                    ...
                    object_counter1[obj_name] += 1
        # vehicle appears in purple and exits:
        ...
        if getMaskPixelColor(int(center[1]), int(center[0])) == "cyan" and id in dict_purple.keys() and id not in listVehCounted:
            dict_purple.pop(id)
            listVehCounted.append(id) #not counting, but just so we don't add it later.
