# v8 Similar to v4, but with purple. Purple at edge, with yellow just above it.
...
# CYAN -> PURPLE also DO NOT COUNT.
# Count vehicle after it spends 1.5 seconds in purple.
# Mostly Batch2. Vehicles of interest appear on the bottom road.
# Edited from v7
# ENTERING: RED -> Green
...
def draw_boxes(img, f_no, bbox, names,object_id, identities=None, offset=(0, 0)):
    ...
    print("\n\n\tPurple keys:\t",dict_purple.keys())
    print("\n\n\tPurple keys:\t",dict_purple)
    ...
    print("\n\n******* TEMP TEMP TEMP ****\n",temp1,"\n\n")
    print("len(temp1.keys()): ", len(temp1.keys()))
    print("identities: ", identities)
    print("len(list(temp1.keys())): ", len(list(temp1.keys())))
    if len(list(temp1.keys())) > 0:
        for p_key in list(temp1.keys()):
            ...
            if p_key not in identities and len(identities) > 0:
                ...
                if abs(max(dict_purp_Y[p_key]) - min(dict_purp_Y[p_key])) < 600 and len(dict_purp_Y[p_key]) >= int(fps_vid/1.5):
                    print("\n\nINSIDE    \t INSIDE  \ INSIDE \n\n\n\n\n")
                    if len(dict_purp_Y[p_key]) < int(fps_vid/5): #f_no > fps_vid*2 and
    ...
    for i, box in enumerate(bbox):
        ...
        # vehicle appears in purple and eventually exits:
            ...
            #For bus
            if abs(x1 - x2) > 520: #520 for BATCH5#250:
                #listVehCounted.append(id)
                continue
            ...
            #Id vehicle in cyan previously, ignore it
            if id in dict_cyan.keys():
                listVehCounted.append(id) #so we don't add it/count by mistake later.
                dict_purple.pop(id)
                continue
            ...
            if len(dict_purple[id]) > 45 and abs(max(dict_purp_Y[id]) - min(dict_purp_Y[id])) < 250:#int(fps_vid*1.5):
                dict_purple.pop(id)
                with open(csv_file_event, 'a+') as f:
                        curr_time = time_of_recording + datetime.timedelta(seconds=round(f_no / fps_vid))
                        f.write(str(curr_time)+","+obj_name+","+str(id)+",Direction1," + file_name_vid + '\n') #Leaving
        ...
        if getMaskPixelColor(int(center[1]), int(center[0])) == "cyan":
            if id not in dict_cyan.keys():
                dict_cyan[id] = [obj_name]
            else:
                dict_cyan[id].append(obj_name)
            '''
            #Ignoring all Purple -> Cyan
            if id in dict_purple.keys() and id not in listVehCounted:
                dict_purple.pop(id)
                listVehCounted.append() #just so we dont count it later.
            '''
