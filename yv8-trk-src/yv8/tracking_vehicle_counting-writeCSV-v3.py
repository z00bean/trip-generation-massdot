...
#usps
# python tracking_vehicle_counting-writeCSV-v3.py mode="pedict" model=yolov8x.pt source=/media/zubin/Stuff1/DATA/TRANSPORT/IR-sample-data/9.BATCH_1/ID373-202303211345to202303231030/100_compressed_vid/03210003.MOV conf=0.55 show=True agnostic_nms=0.4 cls=[0,1,2,3,5,7] line_thickness=1
#374E-03210007.MOV

#4.kids
# python tracking_vehicle_counting-writeCSV-v3.py mode="pedict" model=yolov8s.pt source=/media/zubin/Stuff1/DATA/TRANSPORT/IR-sample-data/9.BATCH_1/ID373-202303211345to202303231030/100_compressed_vid/03210003.MOV conf=0.55 show=True agnostic_nms=0.4 cls=[0,1,2,3,5,7] line_thickness=1
...
def draw_boxes(img, f_no, bbox, names,object_id, identities=None, offset=(0, 0)):
    ...
    '''
    _existing_tracks = []
    for _trk in deepsort.tracker.tracks:
        #print(type(_trk.track_id))
        #print(_trk.track_id)
        _existing_tracks.append(_trk.track_id)
    if identities is not None:
        for _id in trackID_list:
            if _id not in _existing_tracks:
                trackID_list.remove(_id)
                if _id in dict_red.keys(): # Leaving | Diection1
                    obj_name = dict_red[_id]
                    with open(csv_file_event, 'a+') as f:
                        curr_time = time_of_recording + datetime.timedelta(seconds=round(f_no*3 / fps_vid))
                        f.write(str(curr_time)+","+obj_name+","+str(_id)+",Direction1," + file_name_vid + '\n') #Leaving
                    if obj_name not in object_counter: #obj_name is class of object
                        object_counter[obj_name] = 1
                    else:
                        object_counter[obj_name] += 1
                elif _id in dict_green.keys(): # Entering | Diection2
                    obj_name = dict_green[_id]
                    with open(csv_file_event, 'a+') as f:
                        curr_time = time_of_recording + datetime.timedelta(seconds=round(f_no / fps_vid))
                        f.write(str(curr_time)+","+obj_name+","+str(_id)+",Direction2," + file_name_vid + '\n') #Entering
                    if obj_name not in object_counter1:
                        object_counter1[obj_name] = 1
                    else:
                        object_counter1[obj_name] += 1
                removed_trkIDlst.append(_id)
    '''
    ...
    #Todo: get color changes twice, compute direction.
    ...
    for i, box in enumerate(bbox):
        ...
        center = (int((x2+x1)/ 2), int((y2+y2)/2)) #(y2+y2)/2 #x, y : in terms of cv2, vertical is y [0,720).
...
def predict(cfg):
    ...
    cfg.project = os.path.join("/media/zubin/Stuff1/DATA/TRANSPORT/IR-sample-data/OUT_DIR_9", cfg.source.split('/')[-4], cfg.source.split('/')[-3]) #"/media/zubin/Stuff1/DATA/TRANSPORT/IR-sample-data/OUT_DIR_9/Batch1"
    ...
    csv_file_event = os.path.join("/media/zubin/Stuff1/DATA/TRANSPORT/IR-sample-data/OUT_DIR_9", cfg.source.split('/')[-4], str(cfg.source.split('/')[-3][:-4]+".txt"))
