...
# python tracking_vehicle_counting-writeCSV.py mode="pedict" model=yolov8x.pt source=/media/zubin/Stuff1/DATA/TRANSPORT/IR-sample-data/7.count-motion-vehicle/373-03210012.MOV conf=0.5 show=True agnostic_nms=0.4 cls=[0,1,2,3,5,7] line_thickness=1
#usps
# python tracking_vehicle_counting-writeCSV.py mode="pedict" model=yolov8x.pt source=/media/zubin/Stuff1/DATA/TRANSPORT/IR-sample-data/7.count-motion-vehicle/374E-03210020.MOV conf=0.5 show=True agnostic_nms=0.4 cls=[0,1,2,3,5,7] line_thickness=1
#374E-03210007.MOV

# python tracking_vehicle_counting.py mode="pedict" model=yolov8x.pt source=/media/zubin/Stuff1/DATA/TRANSPORT/IR-sample-data/7.count-motion-vehicle/374E-03210003.MOV conf=0.5 show=True agnostic_nms=0.4 cls=[0,1,2,3,5,7] line_thickness=1
#374E-03210046.MOV

#4.kids
# python tracking_vehicle_counting-writeCSV.py mode="pedict" model=yolov8s.pt source=/media/zubin/Stuff1/DATA/TRANSPORT/IR-sample-data/7.count-motion-vehicle/408W-100-03170002.MOV conf=0.5 show=True agnostic_nms=0.4 cls=[0,1,2,3,5,7] line_thickness=1

#5.cop
# python tracking_vehicle_counting-writeCSV.py mode="pedict" model=yolov8s.pt source=/media/zubin/Stuff1/DATA/TRANSPORT/IR-sample-data/7.count-motion-vehicle/xx-761R-01010029.MOV conf=0.5 show=True agnostic_nms=0.4 cls=[0,1,2,3,5,7] line_thickness=1

#408E-03170024.MOV, 6.res-trk
# python tracking_vehicle_counting-writeCSV.py mode="pedict" model=yolov8s.pt source=/media/zubin/Stuff1/DATA/TRANSPORT/IR-sample-data/7.count-motion-vehicle/373-03210012.MOV conf=0.5 show=True agnostic_nms=0.4 cls=[0,1,2,3,5,7] line_thickness=1
...
trackID_list = [] # list of all tracked objects till current time
...
def draw_boxes(img, f_no, bbox, names,object_id, identities=None, offset=(0, 0)):
    ...
    for i, box in enumerate(bbox):
        ...
        if len(data_deque[id]) >= 2:
          ...
          if intersect(data_deque[id][0], data_deque[id][1], line[0], line[1]) and id not in trackID_list: #zubin: added after and...
              trackID_list.append(id) #zubin
              ...
              if "South" in direction:
                with open(csv_file_event, 'a+') as f:
                    ...
                    curr_time = time_of_recording + datetime.timedelta(seconds=round(f_no*3 / fps_vid))
                    f.write(str(curr_time)+","+obj_name+","+str(id)+",Leaving," + file_name_vid + '\n')
              ...
              if "North" in direction:
                with open(csv_file_event, 'a+') as f:
                    ...
                    f.write(str(curr_time)+","+obj_name+","+str(id)+",Entering," + file_name_vid + '\n')
    ...
    #4. Display Count in top right corner
        for idx, (key, value) in enumerate(object_counter1.items()):
            ...
            cv2.putText(img, f'Entering', (width - 300, 35), 0, 1, [0, 0, 180], thickness=1, lineType=cv2.LINE_AA)   #entering main
            cv2.putText(img, cnt_str, (width - 300, 75 + (idx*40)), 0, 1, [0, 0, 180], thickness = 1, lineType = cv2.LINE_AA)   #entering main

        for idx, (key, value) in enumerate(object_counter.items()):
            cnt_str1 = str(key) + ":" +str(value)
            cv2.putText(img, f'Leaving', (11, 35), 0, 1, [180, 180, 180], thickness=1, lineType=cv2.LINE_AA)    #leaving main
